...
# (i.e. if the Dow Jones is 25316.53, then the seed would be 1653.), plus 8.
...
r <- foreach(t = 1:sim, .combine='cbind', .inorder=FALSE, .packages=vec.pac) %dopar% {
  ...
  res <- c(as.vector(results_test), as.vector(results), as.vector(results_het), as.vector(bestML), as.vector(table.who)) # xxx took out results group
  dim(res)
  print(t)
  r <- data.frame(res)
}
