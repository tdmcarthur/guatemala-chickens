...
a           <- as.data.frame(model.matrix(~data$stratum-1)) # Creates data frame with dummies for all levels of data$paire except one
...
names <- c("Height-for-age (Z-score)", "Weight-for-age (Z-score)", "Not stunted (0/1)", "Not underweight (0/1)", "Consumed animal-source foods in past day (0/1)", "Dietary diversity score")    # vector of labels for outcome variables
Y     <- c("zlen", "zwei","notstunted", "notunderweight", "asf", "dds")     # vector of outcome variables
...
fixed_effect <- "0"         # if no fixed_effect  use    fixed_effect <- "0". These should be the strata. 
...
controls     <- c("mujer", "edad_en_meses", "micronutr", "bono", "gpsaltitude", "min_dist", "rain_maydec2015","max30_maydec2015", "AME", "dep_ratio", "edu", "hhwealth", "dirtfloor", "hhsoc_capital", "credit", "market_distance", "electric", "water", "Lcalorias_percap", "LASFproteina_percap", "LFCS", "Lmaize_production", "Lfrijol_production", "Legg_price", "Legg_production", "Lchicken_num", "chicken_shelter", "womanwealth", "womansoc_capital")
...
#controls     <- c("mujer", "nochild", "lag_zwei", "lag_zlen","edad_en_meses", "micronutr", "bono", "gpsaltitude", "min_dist", "rain_maydec2015","max30_maydec2015", "AME", "dep_ratio", "edu", "hhwealth", "dirtfloor", "hhsoc_capital", "credit", "market_distance", "electric", "water", "Lcalorias_percap", "LASFproteina_percap", "LFCS", "Lmaize_production", "Lfrijol_production", "Legg_price", "Legg_production", "Lchicken_num", "chicken_shelter", "womanwealth", "womansoc_capital")
#controls     <- c(controls, names(data)[(substring( names(data), 1, 5)=="paire")]) # adds the village-pair fixed effects to set of control variables
affected       <- c("mujer", "edad_en_meses", "micronutr", "bono", "gpsaltitude", "rain_maydec2015", "max30_maydec2015", "AME", "dep_ratio", "edu", "hhwealth", "dirtfloor", "hhsoc_capital", "credit", "market_distance", "electric", "water", "Lcalorias_percap", "LASFproteina_percap", "LFCS", "Lmaize_production", "Lfrijol_production", "Legg_price", "Legg_production", "Lchicken_num", "chicken_shelter", "womanwealth", "womansoc_capital")      # characteristics for most/least affected analysis
names_affected <- c("Girls (0/1)","Age in months", "Gives micronutrient supplement to children (0/1)", "Receives CCT (0/1)", "Meters above sea level", "Rainfall at closest weather station below median for 2015 crop season (0/1)", "Days with maximum temperature $>$ \ang{30}C during 2015 crop season above median", "Household size (adult male equivalents)", "Dependency ratio", "Average years of education, 12 years of age and older", "Wealth", "Dwelling has dirt floor", "Social capital index, household", "Had credit at baseline (0/1)", "Time in minutes to arrive at nearest market", "Connected to electricity network (0/1)", "Connected to water network (0/1)", "Daily calories per adult male equivalent (lag) (log)", "Daily  grams of animal protein per adult male equivalent (lag) (log)", "Food consumption score (log)", "Maize production, pounds (lag) (log)", "Bean production, pounds (log)", "Egg price (log)", "Eggs produced in last six months (log)", "Chickens owned (lag) (log)", "Chickens have housing (0/1)", "Women's share of wealth", "Social capital index, women")
...
best1 <- order(-bestML_all[1,])[1:2] #the order() function gives the indices of elements ranked from largest to smallest. 
# So the line above takes the first row of bestML_all and both columns, multiplies them by negative 1, and gives us the index of the smallest element and then the second smallest.
best2 <- order(-bestML_all[2,])[1:2]
# all based on first outcome
# Random forest fit best
if(best1[1]!=best2[1]){ best <- c(best1[1],best2[1])} # if they aren't the same go with first measure of g.o.f.
...
write.csv(results_all2,(paste(data.path,"/child_results_all2.csv",sep=""))) # save it to a csv
write.csv(results_het_all2,(paste(data.path,"/child_results_het_all2.csv",sep=""))) # save it to a csv
write.csv(test_all2,(paste(data.path,"/child_test_all2.csv",sep=""))) # save it to a csv
write.csv(bestML_all,(paste(data.path,"/child_bestML_all.csv",sep=""))) # save it to a csv
write.csv(table.who_all2,(paste(data.path,"/child_table.who_all2.csv",sep=""))) # save it to a csv
...
for(l in 1:length(Y)){
  ...
  for(i in 1:length(affected)){
    for(j in 1:length(best)){
      ...
      k <- best[j] # if both g.o.f. measures in agreement for first outcome, list ML method cohsen by second g.o.f. measure first, then second most preferred.
      # if g.o.f. measures not in agreement, show ML method picked by first g.o.f. measure, then second most rpeferred by same measures.
      ...
      # rows 1 through 9, column 2 of tablel.who_all2, which is turned into a 3 x 3
...
write.csv(CLAN_final,(paste(data.path,"/child_CLAN_final.csv",sep=""))) # save it to a csv
write.csv(GATES_final,(paste(data.path,"/child_GATES_final.csv",sep=""))) # save it to a csv
write.csv(BLP_final,(paste(data.path,"/child_BLP_final.csv",sep=""))) # save it to a csv
write.csv(BEST_final,(paste(data.path,"/child_BEST_final.csv",sep=""))) # save it to a csv
